...
if (NOT DEFINED XY_SOC_VER)
	set(XY_SOC_VER 1)  #0:1200对标B0模组;#1:1200(S)模组;2:1200(S) OPENCPU;3:2100S模组;4:2100S OPENCPU;#5:1200(S) 轻量级OPENCPU，暂不支持，请使用配置2;
endif ()

set(VER_BC95 1)                           #需要与CP核保持一套AT命令集。1表示对标BC95的AT命令
...
if(${XY_SOC_VER} EQUAL 0)   #1200对标B0模组
	...
	list(APPEND PRJ_DEF_LIST "LLPM_VER=0")       #极低功耗开关
    ...
    set(XY_SOFTWARE_VERSION "V1200FB10002R00C0005P02_B0")
elseif(${XY_SOC_VER} EQUAL 1)   #1200(S)模组
	list(APPEND PRJ_DEF_LIST "MODULE_VER=1")#模组产品形态 
	...
	list(APPEND PRJ_DEF_LIST "AP_HCLK_DIV=10")   #模组形态使用PLL的AP分频系数，目前仅支持10，值为PLL/分频系数=36.8M
	list(APPEND PRJ_DEF_LIST "DEBUG_MODULE_RUN_TIME=0")  #模组形态使用，精度ms
    set(XY_SOFTWARE_VERSION "V1200FB10002R00C0005P02")
elseif(${XY_SOC_VER} EQUAL 2)   #1200(S) OPENCPU，如表计类产品
	list(APPEND PRJ_DEF_LIST "MODULE_VER=0")#OPENCPU产品形态
    list(APPEND PRJ_DEF_LIST "AT_WAKEUP_SUPPORT=1")  #是否支持AT唤醒。0：不支持，1：支持; OPENCPU版本只能选1
	list(APPEND PRJ_DEF_LIST "BAN_WRITE_FLASH=1")  #禁止CP实时写flash，解决AP核长时间挂起及中断函数不能运行在FLASH上的限制
	list(APPEND PRJ_DEF_LIST "LLPM_VER=1")
	set(CONFIG_FLASH_GIGA_2M "y")# y:2M_FLASH；n:4M_FLASH
	list(APPEND PRJ_DEF_LIST "SYS_CLK_SRC=1")    #系统时钟选择，目前仅支持1或3取值。0:32K_CLK 1:HRC,26M 2:XTAL 3:PLL,368.64M 
	list(APPEND PRJ_DEF_LIST "RC26MCLKDIV=4")    #对HRC进行分频，得到AP核主频，值为26M/分频系数=6.5M
	list(APPEND PRJ_DEF_LIST "AP_HCLK_DIV=1")    #AP核系统时钟使用HRC，不支持二次分频
	set(XY_SOFTWARE_VERSION "V1200SFB10002R00C0005P02_OPEN")
elseif(${XY_SOC_VER} EQUAL 3)   #2100S模组
	list(APPEND PRJ_DEF_LIST "MODULE_VER=1")#模组产品形态 
    list(APPEND PRJ_DEF_LIST "AT_WAKEUP_SUPPORT=1")  #是否支持AT唤醒。0：不支持，1：支持; OPENCPU版本只能选1
	list(APPEND PRJ_DEF_LIST "BAN_WRITE_FLASH=0")
	list(APPEND PRJ_DEF_LIST "LLPM_VER=0")
	set(CONFIG_FLASH_GIGA_2M "n")# y:2M_FLASH；n:4M_FLASH
	list(APPEND PRJ_DEF_LIST "SYS_CLK_SRC=3")   #系统时钟选择，目前仅支持1或3取值。0:32K_CLK 1:HRC,26M 2:XTAL 3:PLL,368.64M 
	list(APPEND PRJ_DEF_LIST "RC26MCLKDIV=2")   #当SYS_CLK_SRC选择PLL时，该参数无意义
	list(APPEND PRJ_DEF_LIST "AP_HCLK_DIV=10")  #模组形态使用PLL的AP分频系数，目前仅支持10，值为PLL/分频系数=36.8M
	list(APPEND PRJ_DEF_LIST "DEBUG_MODULE_RUN_TIME=0")  #模组形态使用，精度ms
	set(XY_SOFTWARE_VERSION "V2100SFB10002R00C0005P02")
elseif(${XY_SOC_VER} EQUAL 4)   #2100S OPENCPU，默认支持动态内存
	list(APPEND PRJ_DEF_LIST "MODULE_VER=0")#OPENCPU产品形态
    list(APPEND PRJ_DEF_LIST "AT_WAKEUP_SUPPORT=1")  #是否支持AT唤醒。0：不支持，1：支持; OPENCPU版本只能选1
	list(APPEND PRJ_DEF_LIST "BAN_WRITE_FLASH=1")  #禁止CP实时写flash，解决AP核长时间挂起及中断函数不能运行在FLASH上的限制
	list(APPEND PRJ_DEF_LIST "LLPM_VER=1")
	...
	list(APPEND PRJ_DEF_LIST "RC26MCLKDIV=4")    #对HRC进行分频，得到AP核主频，值为26M/分频系数=6.5M
	list(APPEND PRJ_DEF_LIST "AP_HCLK_DIV=1")    #AP核系统时钟使用HRC，不支持二次分频
	set(XY_SOFTWARE_VERSION "V2100SFB10002R00C0005P02_OPEN")
elseif(${XY_SOC_VER} EQUAL 5)   #暂未商用！1200 轻量级OPENCPU，支持AP的快速恢复，主频为PLL/10=36.8M
	list(APPEND PRJ_DEF_LIST "MODULE_VER=0")#OPENCPU产品形态
    list(APPEND PRJ_DEF_LIST "AT_WAKEUP_SUPPORT=1")  #是否支持AT唤醒。0：不支持，1：支持; OPENCPU版本只能选1
	list(APPEND PRJ_DEF_LIST "BAN_WRITE_FLASH=1")  #禁止CP实时写flash，解决AP核长时间挂起及中断函数不能运行在FLASH上的限制
	list(APPEND PRJ_DEF_LIST "LLPM_VER=0")       #关闭极低功耗定制化代码
	set(CONFIG_FLASH_GIGA_2M "y")# y:2M_FLASH；n:4M_FLASH
	list(APPEND PRJ_DEF_LIST "SYS_CLK_SRC=3")    #系统时钟选择，目前仅支持1或3取值。0:32K_CLK 1:HRC,26M 2:XTAL 3:PLL,368.64M 
	list(APPEND PRJ_DEF_LIST "RC26MCLKDIV=2")    #当SYS_CLK_SRC选择PLL时，该参数无意义
	list(APPEND PRJ_DEF_LIST "AP_HCLK_DIV=10")   #使用PLL的AP分频系数，目前仅支持10，值为PLL/分频系数=36.8M
	set(XY_SOFTWARE_VERSION "V1200FB10002R00C0005P02_OPEN")
